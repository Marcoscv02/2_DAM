...
public class ClienteEcho {
    public static void main(String[] args) throws IOException {
        ...
        try (Socket echoSocket = new Socket(nombreHost, numeroPuerto); // Socket es un flujo de bytes
             PrintWriter out = new PrintWriter(echoSocket.getOutputStream(), true); // PrintWriter es un flujo de Java.IO.caracteres que envía datos a un flujo de bytes. true para autoflush. Escribirá en el flujo de bytes cada vez que se llame a println
             BufferedReader in = new BufferedReader(new InputStreamReader(echoSocket.getInputStream())); // InputStreamReader es un puente byte a carácter, leemos bytes del flujo de bytes y los convertimos a Java.IO.caracteres
